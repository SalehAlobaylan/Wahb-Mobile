...
export function ProfileScreen() {
  ...
  const [draft, setDraft] = useState<{ username: string; bio: string } | null>(
    null,
  );
  const username = draft?.username ?? profile.data?.username ?? '';
  const bio = draft?.bio ?? profile.data?.bio ?? '';
  const update = useMutation({
    mutationFn: () => clients.iam.updateProfile({ username, bio }),
    onSuccess: () => {
      setDraft(null);
        ...
        {profile.isLoading ? (
          ...
          <>
            ...
            <Pressable
              ...
              style={[styles.save, update.isPending && styles.disabled]}
            >
              <Text style={styles.saveText}>{t('profile.save')}</Text>
            </Pressable>
            {update.isSuccess ? (
              <Text style={styles.notice}>{t('profile.saved')}</Text>
            ) : null}
          </>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  bio: { minHeight: 120, paddingTop: spacing.sm, textAlignVertical: 'top' },
  save: {
    alignItems: 'center',
