...
export function InterestsScreen() {
  ...
  const preferences = useQuery({
    queryKey: ['preferences', subject?.id],
  ...
  const selected =
    selectionOverride ??
    preferences.data?.declared.map((topic) => topic.id) ??
    [];
  const save = useMutation({
    mutationFn: () => clients.cms.updateDeclaredTopics(selected),
  ...
  return (
    <SafeAreaView style={styles.root}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Text style={styles.title}>{t('profile.interestsTitle')}</Text>
        <Text style={styles.copy}>{t('profile.interestsCopy')}</Text>
        {picker.isLoading || preferences.isLoading ? (
          <ActivityIndicator color={colors.pressRed} />
        ) : (
          <View style={styles.topics}>
            {picker.data?.topics.map((topic) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {i18n.language === 'ar' ? topic.label_ar : topic.label_en}
                  </Text>
                </Pressable>
              );
            })}
        ...
        {preferences.data?.muted_sources.length ? (
          <View style={styles.mutedSources}>
            <Text style={styles.mutedTitle}>{t('profile.mutedSources')}</Text>
            <Text style={styles.mutedCopy}>
              {t('profile.mutedSourcesCopy')}
            </Text>
            {preferences.data.muted_sources.map((source) => (
              <Pressable
                accessibilityLabel={`${t('profile.restoreSource')}: ${source.source_key.replace(/^(feed|name):/, '')}`}
                accessibilityRole="button"
                key={source.source_key}
                onPress={() => restoreSource.mutate(source.source_key)}
                style={styles.restoreSource}
              >
                <Text numberOfLines={1} style={styles.restoreSourceKey}>
                  {source.source_key.replace(/^(feed|name):/, '')}
                </Text>
                <Text style={styles.restoreSourceAction}>
        ...
        >
          <Text style={styles.saveText}>{t('profile.saveInterests')}</Text>
        </Pressable>
      </ScrollView>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  topicActive: { backgroundColor: colors.ink },
  topicText: {
    color: colors.ink,
    fontFamily: fontFamilies.bodyMedium,
    fontSize: 14,
  },
  topicTextActive: { color: colors.inkInverse },
  save: {
    alignItems: 'center',
